...
- The full audit chain — a per-platform Trivy HIGH/CRITICAL gate over the exact Docker archives, CycloneDX SBOMs, archive checksum verification, and a smoke test — exists only in the manually triggered `Build, test, and audit` workflow, and that workflow publishes nothing: the formal release job it feeds is not enabled yet (`PUBLISH.sha256` is reserved for it). The `docker-publish` workflow on pushes to `main` only gates on tests, vet, and frontend typecheck, then has buildx push a multi-arch image straight to GHCR under mutable `:latest` / `:sha-<short>` tags — no per-platform scan, no SBOM, no publish environment. Production deployments must pin `image@sha256:...` digests. Image archives are unsigned; Sigstore verification is not implemented and remains a known risk.
- A one-time operator action is required before the first publish: set the repository workflow default to `Read and write permissions`, and allow workflows to write to the `ghcr.io/kingsunb/novaveil` package. Without this, GitHub returns `denied: permission_denied: write_package` and the publish step aborts after the build succeeds.
