...
5. **凭据安全红线（后续审计验收约束）**：API_KEY/TOKEN/SECRET/JWT 等不得通过命中明细重新进入日志原文；命中明细下发命中原文片段（非整份请求体），可见性边界与日志详情/错误日志原有管理员可见性一致，并受条数/字节上限与截断标记约束；如需 preview + sha256，须另行评估预览泄露与低熵原文可枚举的风险。当前尚无命中事件库及这类摘要管线，不能把现有错误日志 redact 等同于已实现该方案。
