...
6. **确定性请求错误快速失败：** 上游返回 400/404/422，或无密钥渠道（未配置任何凭据）被上游 401/403 拒绝时，该成员不再重试、不等待冷却，当场计失败并切换下一优先级；全部启用成员都各失败一次后立即终止，把明确错误交还下游（统一 `暂无可用渠道`）。
...
```text
          ...
          对可探测渠道并行测试      到期后进入 HALF_OPEN
             ...
             有渠道探测成功          本批全部失败
                     ↓                     ↓
              保持 HALF_OPEN 等业务确认       失败渠道重新 OPEN
                     ↓                     ↓
...
| 字段 | 类型 | 新分组默认值 | 说明 |
| --- | --- | --- | --- |
| `member_max_attempts` | int | 3 | 单个成员包含首次请求的总尝试次数，仅在故障转移模式生效。确定性请求错误（400/404/422、无密钥渠道的 401/403）不按此重试：每成员只派发一次。 |
...
| `member_retry_interval_seconds` | int | 2 | 同一成员相邻两次尝试之间的等待秒数。没有可选成员，或引用结构性跳过过多时，也用它做退避。确定性请求错误不等待该间隔，直接切换下一成员。 |
...
1. **会话识别：** 优先取请求头 `X-Session-Id`；没有时回退到 `x-opencode-session`。携带相同标识的请求视为同一会话。粘合、OpenCode 会话号和脱敏共用 `sessionScopeKey`：有 API Key 时是 `id:session`，控制台是 `console:` 前缀。原始头本身不单独做键。
2. **缺失即不粘合：** 两个头都没有时不建立任何粘合，按正常顺序扫描成员。
...
确定性请求错误（400/404/422，以及无密钥渠道的 401/403）在上表两类之外：既不算引用跳过，也不按瞬态失败退避重试——每成员只派发一次，计失败并清粘合后立即切换下一成员，全败即终止并返回统一 `暂无可用渠道`。
...
| 确定性请求错误快速失败（400/404/422、无密钥渠道 401/403）：每成员一次、不重试不等待、全败即终止返回统一错误 | `internal/relay/handler.go`（主循环） |
