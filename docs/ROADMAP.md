...
- 凭据列 `nv1:` 静态加密覆盖渠道 Key、多 Key、渠道代理和 API Key。列表接口对渠道 Key 与多 Key 打码，渠道代理保留明文。数据库备份导出的渠道 Key 与 API Key 是明文，代理和自定义头值仍打成 `****`。
...
库内渠道 Key、多 Key、自定义头、头模板、JWT 密钥、全局代理和渠道代理都是 `nv1:`。列表对渠道 Key 只出掩码，渠道代理出明文。数据库备份里的渠道 Key 与 API Key 是明文；代理和自定义头值仍是 `****`。`channel_proxy` 与 `BaseURL` 均不做目标地址范围限制，内网代理/内网上游都可直连。导入拒绝精确 `****`。

前端眼睛按钮不把密钥放进 `useQuery`。渠道保存和创建 API Key 的明文也不进 mutation state。渠道代理在列表与编辑页直接显示完整明文。

- [x] 上述列已加密。数据库备份的渠道 Key 与 API Key 是明文。渠道文本导出包含全部渠道（含内置渠道和没有 Key 的渠道），写出明文渠道名、明文 BaseURL 和明文 Key。列表接口对渠道 Key 只出掩码，渠道代理出明文。
- [x] 保存和创建成功后清掉 mutation 里的明文，创建结果只留在组件状态。渠道代理明文经列表进入 React Query 缓存（管理员界面直接显示）。
