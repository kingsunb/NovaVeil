...
> **2026-09-22 部分翻转**：「每次登录均 24h」已改为布尔二选一——勾选「信任此设备」→ 持久 24h cookie；取消 → 会话 cookie（浏览器关闭即失效）。JWT `exp` 仍恒 24h，且仍不接受客户端自报任意时长。见 [恢复登录复选框](../feature/2026-09-22-restore-login-remember-device-checkbox.md)。
...
> **2026-09-22 部分翻转**：「登录页不再提供时长选择」已改为提供「信任此设备」布尔二选一（持久 24h / 单次会话），见 [恢复登录复选框](../feature/2026-09-22-restore-login-remember-device-checkbox.md)。「不接受客户端自报任意时长、JWT/cookie 固定 24h 上限」的核心边界不变。
